...
def _ensure_tmux_session(session_name: str = "araras_monitor") -> Tuple[Any, Any]:
    """Return panes for the shared monitoring tmux session, attaching if needed.

    This helper lazily creates a tmux session on first use. The session contains
    two evenly split panes: the first is used to run the monitored process and
    the second runs the crash monitor script. When the session is created it is
    attached to the current terminal so users can immediately see both panes.

    Args:
        session_name: Name for the tmux session.

    Returns:
        Tuple with the process pane followed by the monitor pane.

    Raises:
        ImportError: If :mod:`libtmux` is not installed.
    """
    ...
    if _TMUX_SESSION is None:
        ...
        try:
            _TMUX_SERVER.attach_session(session_name=session_name)
        except Exception:
            pass
