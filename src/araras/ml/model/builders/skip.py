    ...
    *,
    ...
    if strategy == "final":
        ...
        for i in range(last_idx):
            ...
            if include:
                src = project(layers_list[i], layers_list[-1], f"{name_prefix}_{i}_{last_idx}")
                selected.append(src)
        if not selected:
            return layers_list[-1]
    ...
    for j in range(1, N):
        ...
        for i in range(j):
            ...
            if include:
                src = project(updated[i], updated[j], f"{name_prefix}_{i}_{j}")
                sources.append(src)
        if not sources:
            continue
...
def trial_skip_connection(
    ...
    *,
    ...
    projected = _project_dense(source, target, use_batch_norm, name_prefix)

    if merge_mode == "concat":
        return layers.Concatenate(
            axis=axis_to_concat, name=f"{name_prefix}_concat"
        )([projected, target])
    if merge_mode == "add":
        return layers.Add(name=f"{name_prefix}_add")([projected, target])
    raise ValueError(f"Unknown merge_mode '{merge_mode}'. Use 'concat' or 'add'.")
