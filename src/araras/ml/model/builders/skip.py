...
def trial_skip_connection(
    ...
    *,
    ...
    """Conditionally connect ``source`` to ``target`` with projection.

    Layer names generated by this function are guaranteed to be unique, enabling
    repeated invocations within a single model without naming conflicts.

    This helper is intended for Graph Neural Network (GNN) architectures where
    the number of nodes remains constant but the channel dimension may vary
    between layers.  The function samples whether to include the skip connection
    using ``trial.suggest_categorical``.  When enabled, ``source`` is projected
    with a dense layer (and optional batch normalization) so its channel
    dimension matches ``target``.  The projected tensor is then merged with
    ``target`` via addition or concatenation.

    Args:
        trial: Optuna trial controlling whether the skip is included.
        source: Tensor providing the skip connection.
        target: Tensor that receives the skip projection.
        axis_to_concat: Axis used when ``merge_mode`` is ``"concat"``.
        use_batch_norm: If ``True``, apply batch normalization after the
            projection layer.
        merge_mode: ``"add"`` or ``"concat"``.
        name_prefix: Prefix for generated layer names.

    Returns:
        The merged tensor if the skip connection is activated, otherwise the
        unmodified ``target`` tensor.

    Raises:
        ValueError: If ``merge_mode`` is not one of ``"add"`` or ``"concat"``.
    """

    include = trial.suggest_categorical("skip", [False, True])
    if not include:
        return target

    projected = _project_dense(
        source, target, use_batch_norm, _unique_name(name_prefix)
    )

    if merge_mode == "concat":
        layer_name = _unique_name(f"{name_prefix}_concat")
        return layers.Concatenate(axis=axis_to_concat, name=layer_name)([
            projected,
            target,
        ])
    if merge_mode == "add":
        layer_name = _unique_name(f"{name_prefix}_add")
        return layers.Add(name=layer_name)([projected, target])
    raise ValueError(f"Unknown merge_mode '{merge_mode}'. Use 'concat' or 'add'.")
