...
def get_callbacks_study(
    ...
    reduce_lr_min_lr: float = 1e-10,
    pruning_interval: int | None = 1,
    verbose: int = 0,
    ...
    Args:
        ...
        monitor (str): The metric to monitor for early stopping, learning rate
            reduction and pruning callbacks.
        ...
        mode (str): One of ``"auto"``, ``"min"``, or ``"max"`` to control how
            monitored metrics are interpreted by ``EarlyStopping`` and
            ``ReduceLROnPlateau``. Defaults to ``"auto"``.
        reduce_lr_factor (float): Factor by which the learning rate is reduced.
            Defaults to ``0.2``.
        reduce_lr_min_lr (float): Lower bound on the learning rate. Defaults to
            ``1e-10``.
    ...
    if pruning_interval is not None:
        pruning_callback = KerasPruningCallback(
            trial, monitor=monitor, interval=pruning_interval
        )
    #! ———————————————————————————————————————————————————————————————————————————— !#
