"""Email management utilities for the monitoring system."""
...
from .print_utils import print_warning_message, print_error_message
...
__all__ = ["ConsolidatedEmailManager"]


class ConsolidatedEmailManager:
    """Handle consolidated email notifications for restart events."""
    ...
    def _validate_email_config(self) -> bool:
        ...
        if not (recipients_exists and credentials_exists):
            print_warning_message(
                "Email config files not found, email alerts disabled"
            )
            print(
                f"Expected files: {self.recipients_file}, {self.credentials_file}"
            )
            return False
        return True
    ...
    def send_consolidated_status_email(
        ...
        try:
            ...
            if status_type == "restart_success":
                ...
                status_description = (
                    "Process crashed but was successfully restarted"
                )
                ...
                if remaining > 0:
                    subject = (
                        f"{title} crashed - Restart Failed ({failed_attempts} attempts, {remaining} remaining)"
                    )
                    status_description = (
                ...
                status_description = (
                    "Process completed all tasks successfully"
                )
    ...
    def report_final_failure(
        self, title: str, restart_count: int, error: str
    ) -> None:
        self.send_consolidated_status_email(
            "restart_failed",
            {
