...
__all__ = ["FlagBasedRestartManager"]


class FlagBasedRestartManager:
    """Enhanced restart manager with consolidated email notifications."""
    ...
    def run_file_with_restart(
        ...
        if file_type == "notebook":
            print_process_status(
                f"Target file is a notebook: {validated_path.name}. Converting to Python file..."
            )
            try:
                self.converted_python_file = NotebookConverter.convert_notebook_to_python(
                    validated_path
                )
                self.original_was_notebook = True
                print_process_status(
                    f"Notebook {validated_path.name} converted to temporary file: {self.converted_python_file}"
                )
            ...
            while self.running and self.restart_count < self.max_restarts:
                ...
                try:
                    if self.monitor_info:
                        from .monitoring import stop_monitor
                    ...
                    from .monitoring import start_monitor
                    ...
                    if completion_reason == "success_flag":
                        ...
                        print_process_status(
                            "Process crashed, checking restart policy"
                        )
    ...
    def _launch_process(
        ...
        command, _ = FileTypeHandler.build_execution_command(
    ...
    def _discover_target_pid(self, pid_file: str, timeout: float = 5.0) -> Optional[int]:
        ...
        while self.running:
            ...
            try:
                ...
                if check_count % 2 == 0 and self.monitor_info:
                    from .monitoring import check_crash_signal
                ...
                print_process_status(
                    "CTRL+C detected, shutting down monitored process"
                )
                self.running = False
                self._cleanup_all()
                return "interrupted"
