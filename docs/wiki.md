...
| csv_path | `str, optional` | Path to save trial results in CSV format. The CSV includes the sampled parameters and is sorted by training memory in descending order. |
...
- When ``csv_path`` is provided the sampled statistics are saved to CSV. The CSV
  includes each trial's parameters and is sorted by training memory in
  descending order. If ``logs_dir`` is set, parameters of failed trials along
  with their traceback are written to individual log files.
