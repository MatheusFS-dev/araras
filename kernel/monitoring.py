...
def _monitor_worker(
    ...
    Args:
        ...
        credentials_file (str): Path to the JSON file with email credentials.
        log_dir (Optional[str]): Directory for log files; if provided, errors are logged here.
    ...
    while True:
        try:
            ...
            print(f"[ERROR] Monitoring process failed: {e}")
            # Save error to log file if log_dir is provided
            if log_dir:
                os.makedirs(log_dir, exist_ok=True)
                with open(f"{log_dir}/monitoring_errors.log", "a") as log_file:
                    log_file.write(f"[ERROR] {e}\n")
                    log_file.write(traceback.format_exc())
            _send_alert_email(pid, "Kernel Monitor FAILED", custom_title, recipients_file, credentials_file)
            break
...
def start_monitor(
    ...
    p = Process(
        ...
        args=(pid, interval, custom_title, recipients_file, credentials_file, log_dir),
